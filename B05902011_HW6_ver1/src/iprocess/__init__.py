...
class YokoiConnNumber(object):
    def __new__(cls, img, X, Y):
        return [
            str(cls._f(cls, pixels=img, center_x = x, center_y = y)) if img[x, y] != 0 else ' ' for y in range(Y) for x in range(X)
        ]
    ...
    def _f(cls, pixels, center_x, center_y):
        width, height = np.shape(pixels)
        x = list(map(lambda position: pixels[position] if 0 <= position[0] < width and 0 <= position[1] < height else 0,
...
# https://github.com/cyliang, modified to fit my data structure
